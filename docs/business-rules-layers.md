...
Bu doküman, `business-rules.md` içindeki 30 kuralın hangi katmanda (Veritabanı / Servis / Hibrit) uygulandığını gösterir. Kod yazarken veya mimari inceleme yaparken referans olarak kullanılabilir.
...
| BR-002 | User silinirse sahip olduğu Task'lar da silinir | **DB** | FK `ON DELETE CASCADE` (User → Task via OwnerId). **Not:** Task'taki diğer User FK'ları (`CompletedByUserId`, `DeletedByUserId`) `ON DELETE NO ACTION` olmalı — SQL Server multiple cascade paths kısıtlaması nedeniyle manual/code cleanup yapılır. |
...
| BR-011 | Soft-delete Task, restore edilene kadar listelerde görünmez | **Servis** | EF Core `HasQueryFilter(t => !t.IsDeleted)` Global Query Filter |
| BR-012 | Silinmiş Task'a yeni SubTask eklenemez | **Servis** | `AddSubTask`: önce parent `Task.IsDeleted` kontrolü |
...
| BR-020 | SubTask'ın kendi yetki kaydı yok, erişim parent'tan gelir (Paylaşılanlar ekler/tamamlar; silme SADECE Owner) | **Servis** | Yetki kontrolü `TaskAuthorizationService` üzerinden yapılır; silmede `currentUser == Task.OwnerId` şartı aranır |
...
| BR-025 | Ana görevi güncelleme ve tamamlama yetkisi yalnızca Owner'a aittir | **Servis** | `TaskAuthorizationService.EnsureCanModifyAsync` ve `EnsureCanCompleteAsync` |
...
| BR-030 | Görev sahibi sahipliği devredebilir | **Servis** | `TaskTransferService`: onay, ret ve iptal durum akışı |
...
| **Servis** | 17 | BR-004, 008, 009, 010, 011, 012, 013, 017, 018, 020, 022, 023, 025, 026, 028, 029, 030 |
| **Hibrit** | 3 | BR-005, 014, 027 |
| **TOPLAM** | **30** | **30 Kural (%100 Kapsam)** |
